...
f = open ('%s/%s.elf.symtab' % (target_dir, base))
(lsymbs, lsects) = objdump.build_syms (f)
f.close ()
symbols.update (lsymbs)
sections.update (lsects)

f = open ('%s/%s_C_graph.txt' % (target_dir, base))
syntax.parse_and_install_all (f, 'C')
f.close ()

f = open ('%s/%s_mc_graph.txt' % (target_dir, base))
...
f = open ('%s/%s.elf.rodata' % (target_dir, base))
...
def asm_split_pairings ():
	pairs = [(s, isabase + '.' + s) for s in ['icrc','icrc1','main']]
